...
- `php artisan sistema:backup` — backup manual de la BD realcomputer (mysqldump) a `storage/app/backup/realcomputer_YYYYMMDD_HHMMSS.sql`. Alternativa: `backup-base-de-datos.bat` (raíz).
...
- **POS Chatarra (Venta/Compra)**: `ScrapPosController` + vista `scrap_pos/index.blade.php`. Dos POS: `GET /pos-venta` y `GET /pos-compra`, cada uno con 5 **estaciones fijas** (`restaurant_tables.pos_mode = venta|compra`). Reutiliza `RestaurantOrder`/`RestaurantOrderItem`/`RestaurantTable`.
  ...
  - **Stock conectado**: compra suma stock; venta valida stock (`assertSaleStock`, acumulado en la orden). **Configurable** por empresa (`companies.allow_negative_stock`, switch en Empresa → Editar → Configuración de IGV): si está activo, el POS Venta **no bloquea** ventas con stock 0/negativo.
  - **Editar línea**: `updateItem` acepta `quantity`/`quantity_delta`/`notes` y **`price_level` opcional** (recalcula `unit_price` vía `priceVenta/Compra` y el total). El frontend reutiliza el modal de cantidad para editar (botón ✏️ por línea, `editItem()`), con label "Guardar cambios".
...
- **Ingresos y Gastos**: módulo `/ingresos-gastos` (`CashMovementController` + modelo `CashMovement`, tabla `cash_movements`). INGRESO suma, GASTO resta del cuadre de caja en vivo. La lista y totales **solo muestran los movimientos de la caja abierta actual** (`cash_register_id`); con caja cerrada el módulo queda vacío (el historial por caja se ve en `/cashregisters/{id}`).
- **Reportes Compras/Ventas**: módulo `/reportes` (`ReportController` + vista `reports/index.blade.php`). Filtros: **tipo** (venta/compra, CO vs ≠CO), **periodo** (diario/semanal/mensual con navegación ±), **selección** (todos / por categoría / varios productos `productos[]`). KPIs (docs, cantidad, importe, IGV), detalle por producto, resumen diario, lista de comprobantes y **exportación Excel** (`reports.export`). Permiso: `view_invoices`. Agregaciones sobre `invoice_items` join `invoices`/`products`/`categories` en la fecha de `fecha_emision`.
- **Flujo de Caja (cierre)**: `saldo_final = monto_apertura + ventas + ingresos − compras − gastos`. `close()` excluye compras (CO) de las ventas (`paymentBuckets()` compartido). Resumen = Flujo de Caja del Día. Caja abierta muestra saldo en vivo. **El cierre es automático**: el usuario NO ingresa monto; `monto_cierre = saldo_final` (diferencia entre ingresos y egresos).
...
- **Rutas por permiso (no middleware admin)**: las rutas de comprobantes (`/invoices*`, `/sunat-summaries*`, `/documents/{tipo}`, `pos.print`), de caja (`/cashregisters*`, `/cashregister/open|close`) y de **productos** (`/products*`, `/products/import*`, `/products/inventory-report*`, composite, duplicate) se protegen con `$this->authorize('permission', ...)` (`view_products`|`create_products`|`edit_products`|`delete_products`, etc.). El middleware `admin` (IsAdmin) se usa para usuarios/series/empresas/compras/categorías/etc.
